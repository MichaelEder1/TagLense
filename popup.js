document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderLogs(logs) {
    ...
    if (!logs || logs.length === 0) {
      logList.innerHTML = '<div class="logEntry">No logs</div>';
      return;
    }
    logs.forEach(l => {
      ...
      meta.textContent = `${d.toLocaleString()} • ${l.url || ''} • ${l.level || 'info'}`;
      const payload = document.createElement('div');
      payload.className = 'logPayload';
      try { payload.textContent = JSON.stringify(l.message, null, 2); } catch (e) { payload.textContent = String(l.message); }
      el.appendChild(meta);
  ...
  function fetchLogs() {
    chrome.runtime.sendMessage({ action: 'getDebugLogs' }, (resp) => {
      if (resp && resp.logs) renderLogs(resp.logs);
    });
  }

  // Initialize debug toggle from storage
  if (debugToggle) {
    try {
      if (typeof chrome !== 'undefined' && chrome && typeof chrome.storage !== 'undefined' && chrome.storage && typeof chrome.storage.local !== 'undefined') {
        chrome.storage.local.get({ debugForward: false }, (res) => {
          debugToggle.checked = !!res.debugForward;
        });
        debugToggle.addEventListener('change', () => {
          const val = !!debugToggle.checked;
          chrome.storage.local.set({ debugForward: val });
        });
      } else {
        // If storage isn't available, keep toggle off and disable it
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg && msg.action === 'debugLogBroadcast' && msg.payload) {
      // prepend new log
      const current = [];
      const node = document.querySelector('#logList .logEntry');
  ...
  clearBtn.addEventListener('click', () => {
    chrome.runtime.sendMessage({ action: 'clearDebugLogs' }, () => { fetchLogs(); });
  });
